%% 增强版航迹+点迹特征提取脚本
% 功能：根据RD文件名，提取对应航迹点范围的特征
% 改进：
%   1. 结合航迹(Tracks)和点迹(PointTracks)数据
%   2. 提取更多有区分力的特征
%   3. 添加统计特征（均值、方差、变化率）
%
% 输出特征维度: [12, 16] 时序特征 + [1, 20] 统计特征
...
point_txt_folder = uigetdir(pwd, '选择点迹txt文件夹（PointTracks_xxx.txt，可取消跳过）');
    ...
    for label = 0:5
        ...
        for f = 1:length(rd_files)
            ...
            try
                ...
                % 提取特征
                [track_data, track_stats] = extract_enhanced_features(...
...
fprintf('   统计特征维度: [1 x 20]\n');


%% 增强特征提取函数
function [track_data, track_stats] = extract_enhanced_features(track_T, point_T, point_start, point_end, target_len, smooth_win)
    track_data = [];
    track_stats = [];

...
class GesDataLoaderNew(Dataset):  # 需要继承data.Dataset
    def __init__(self, path_name1, path_name2, path_name3, data_rows, data_cols, val=False):
        """
        :param path_name:数据集路径
        """
        self.path1 = path_name1
        self.path2 = path_name2
        self.path3 = path_name3
        ...
        self.label_and_data_file_name3 = []  # 创建一个保存标签与数据文件名的list
        for i in self.label:
            ...
            path3_ = os.path.join(path_name3, i)  # 打开每个标签文件夹
            file_name_list = os.listdir(path3_)  # 获得标签文件夹内所有数据文件名
            for h in file_name_list:
                self.label_and_data_file_name3.append((i, h))  # 把标签和对应的文件名的存在list中
    ...
    def __getitem__(self, index):
        _data = np.zeros((3, self.data_rows, self.data_cols))
        ...
        file_name3 = self.label_and_data_file_name3[index][1]
        ...
        file_path3 = os.path.join(self.path3, its_label, file_name3)
        ...
        original_data = scio.loadmat(file_path3)
        data03 = original_data['data']
        data13 = np.array(data03)

        curr_rows = data13.shape[0]
        if curr_rows > self.data_rows:
            if False and curr_rows == 181:
                data23 = data13[58:122, :]
            else:
                data23 = data13[:self.data_rows, :]
        else:
            num_pad = self.data_rows - data13.shape[0]
            data23 = np.pad(data13, ((0, num_pad), (0, 0)), 'constant', constant_values=(0, 0))

        curr_cols = data13.shape[1]
        if curr_cols > self.data_cols:
            data23 = data23[:, :self.data_cols]
        else:
            num_pad = self.data_cols - data13.shape[1]
            data23 = np.pad(data23, ((0, 0), (0, num_pad)), 'constant', constant_values=(0, 0))

        _data13 = data23.reshape((1, self.data_rows, self.data_cols))
        _data13 = 20 * np.log(_data13 + 1e-7)
